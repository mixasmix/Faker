...
fork from https://github.com/fzaninotto/Faker
...
```sh
composer require mixasmix/faker
```
...
//Generate Russian main state registration number (Генерация ОГРН)
echo $faker->ogrn;

//Generate Russian main state registration number individual interpreter (Генерация ОГРНИП)
echo $faker->ogrnip;

//Generate Russian checking account number Company (Генерирует расчетный счет компании)
//Return object CheckingAccountData with method __toString() stringify value. Method getBik() returned 
//BIK - Bank identification code (Банковский идентификационный код)
$rs = $faker->rs();

echo $rs; //40701810031333634620
echo $rs->getBik(); //049139426
//Constructor takes a value of three digits - currency "All-Russian classifier of currencies". Default 810 - Russian rubles

$rs = $faker->rs('818'); //Egyptian Pound 

echo $rs; //40701818875609437829
echo $rs->getBik(); //042215928

//Generate Russian correspondent account number Company (Генерирует корреспондентский счет компании)
//Return object CheckingAccountData with method __toString() stringify value. Method getBik() returned 
//BIK - Bank identification code (Банковский идентификационный код)
$ks = $faker->ks();

echo $ks; //30110810962516338400
echo $ks->getBik(); //049139426

//Constructor takes a value of three digits - currency "All-Russian classifier of currencies". Default 810 - Russian rubles
$ks = $faker->ks('818'); //Egyptian Pound 

echo $ks; //30110818962516338400
echo $ks->getBik(); //042215928
```
